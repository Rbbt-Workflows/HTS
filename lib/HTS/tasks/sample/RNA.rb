module Sample
  ...
  dep_task :hisat, HTS, :hisat, :fastq1 => :placeholder, :fastq2 => :placeholder do |sample,options|
    sample_files = Sample.sample_files sample
    raise "Sample #{ sample } not found" if sample_files.nil?
  ...
  dep_task :stringtie, HTS, :stringtie do |sample,options,dependencies|
    ...
    dep_hisat = dependencies.flatten.select{|dep| dep.task_name == :hisat }.first
    options = options.merge("HTS#hisat" => dep_hisat)
    {:inputs => options, :jobname => sample}
  end
end
